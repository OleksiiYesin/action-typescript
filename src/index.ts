...
async function run() {
    try {
        if (!dir && fs.existsSync(`${state}`)) {
            await destroyOne();
        } else {
            setFailed('\nFile not exist in 1st case!')
        }

        if (dir && fs.existsSync(`${dir}/${state}`)) {
            await destroy(); 
        } else {
            setFailed('\nFile not exist!')
        }
...
async function destroyOne() {
    try {
        ...
        const stateFile = fs.readFileSync(`${state}`);
        const obj = JSON.parse(stateFile.toString());
        const shareInfoLen = Object.keys(obj.resources).length;

        // destroying resources
        const destroyResources = async () => {return getStdOutput('terraform', [ 'init' ])};
        const destroy = async () => {return getStdOutput('terraform', [ 'destroy', '--auto-approve' ])};
        let attempt = 0;
